...
----------------------------------------------------
-- HỆ THỐNG THỐNG KÊ (STATS MONITOR)
----------------------------------------------------
local startTime = os.time()
local StatsFrame = Instance.new("Frame")
StatsFrame.Name = "StatsFrame"
StatsFrame.Size = UDim2.new(0, 220, 0, 140)
StatsFrame.Position = UDim2.new(0, 20, 0, 65) -- Nằm dưới nút Toggle
...
local InfoLabel = Instance.new("TextLabel", StatsFrame)
...
RunService.RenderStepped:Connect(function(deltaTime)
    ...
    -- Lấy thông tin mục tiêu (Target)
    local targetName = "None"
    local target = Mouse.Target
    if target then
        local model = target:FindFirstAncestorOfClass("Model")
        if model and model:FindFirstChild("Humanoid") then
            targetName = model.Name .. " (NPC/Player)"
        else
...
-- Tạo hiệu ứng viền cho StatsFrame (Tùy chọn)
